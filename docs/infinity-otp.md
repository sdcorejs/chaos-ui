...
The glove uses three original, image-generated transparent artwork frames: a front-facing idle hand, a side view where the armored thumb and middle fingertips meet, and a release pose where they separate. The frames and original SVG warrior bust ship with the core npm package; no film still, remote URL, GIF host or playground asset path is required. The idle glove connects visually to the warrior's raised arm, and the close side view makes the snap readable. Gem cuts are SVG paths with distinct silhouettes at the six sockets. The reusable source has varied cuts and colors chosen independently per instance; neither a digit nor its source color determines the mounted gem shape. The six drop targets remain at least 43 CSS pixels wide in narrow layouts, with distinct printed digits before and after placement. Shadow DOM isolates styling; supported parts are `board`, `header`, `stage`, `titan`, `gauntlet`, `sockets`, `socket`, `input`, `pool`, `stone`, `source`, `hint`, `actions`, `reset`, `remove`, `mode-toggle`, `submit`, `message`, `celebration`, `failed-snap`, `dust`, `drag-stone`.
...
On a transition to error, the numbered stones briefly clear the hand so the side-view finger contact and release are visible; there is no success flash, and the warrior recoils with a surprised "Ơ KÌA?!" reaction. All six digits return in place for correction. On success, the same contact/release sequence has a flash at the fingertips, particles and a temporary ash-like fade of the glove, sockets and stone supply. These visuals return after the animation so the UI remains usable. They are confined to the component's shadow DOM and never remove or alter external nodes. Re-rendering does not restart an effect; reset/unmount cancels it. With `prefers-reduced-motion` or `reducedMotion="always"`, the motion is replaced by static color/text feedback. Audio is off by default, begins only after opt-in and a trusted user interaction, and closes on unmount. No timers, RAF, external DOM changes, 3D engine or remote assets are used. This MVP supports client interaction; SSR rendering/hydration are unverified.
